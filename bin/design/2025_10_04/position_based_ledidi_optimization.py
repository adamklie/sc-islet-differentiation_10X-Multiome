...
print("\nParameters set.")
print(f"\tn_seqs={n_seqs}, n_runs={n_runs}")
...
dir_name = f"ISL1_{chrom_}:{start_}-{end_}_Ledidi_design_{run_hash}"
        ...
        for fl in flank_choices:
            ...
            for y_bar in tqdm(y_bars, desc="Running Ledidi for each decile"):
                ...
                for i in tqdm(range(n_runs)):
                    X_hat_.append(ledidi(count_wrapper.type(torch.float32), X_ref.type(torch.float32), y_bar, n_samples=n_seqs, input_mask=msk, verbose=False))
                X_hat.append(X_hat_)
            X_hat = [torch.stack(x_hat_) for x_hat_ in X_hat]
            X_hat = torch.stack(X_hat)
            print(f"X_hat shape: {X_hat.shape}")
            
            # Create identifiers for each of X_hat based on ISL1_{chrom_}:{start_}-{end_}_Ledidi_run_{run}_decile_{decile}
            identifiers = []
            for decile in range(X_hat.shape[0]):
                for run in range(X_hat.shape[1]):
                    for t, seq in enumerate(range(X_hat.shape[2])):
                        identifiers.append(f"ISL1_{chrom_}:{start_}-{end_}_Ledidi_run-{run+1}_target-{round(y_bars[decile].item(), 2)}_pos-{midpoint}_flanks-{fl}_{t+1}")
            ...
            for j, y_bar in enumerate(y_bars):
                ...
                plt.title(f"Ledidi for {chrom_}:{start_}-{end_} with target {round(y_bar.item(), 2)} positions: {edit_start}-{edit_end}")
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_Ledidi_design_target_{round(y_bar.item(), 2)}_pos-{midpoint}_flanks-{fl}_{run_hash}.pdf"), dpi=300)
                plt.show()
            ...
            np.savez_compressed(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_Ledidi_X_ref_attr_pos-{midpoint}_flanks-{fl}_{run_hash}.npz"), X_ref_attr=X_ref_attr_.numpy(force=True))
            np.savez_compressed(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_Ledidi_X_optim_attr_pos-{midpoint}_flanks-{fl}_{run_hash}.npz"), X_optim_attr=X_optim_attr_.numpy(force=True))
            ...
            seqlets[["id", "start", "end", "attribution", "p-value", "annotation"]].sort_values(["id", "start"]).to_csv(
                os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_Ledidi_seqlets_pos-{midpoint}_flanks-{fl}_{run_hash}.tsv"), 
                sep="\t", 
                index=False
            )
            ...
            delta_seqlets_all.sort_values(["id", "start"]).to_csv(
                os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_Ledidi_delta_seqlets_pos-{midpoint}_flanks-{fl}_{run_hash}.tsv"), 
                sep="\t", 
                index=False
            )
            ...
            # Save edit_df
            edit_df[["id", "chrom", "pos", "ref", "alt", "variant_id", "relative_pos"]].to_csv(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_Ledidi_edits_pos-{midpoint}_flanks-{fl}_{run_hash}.tsv"), sep="\t", index=False)
            ...
            design_df = pd.DataFrame({
                ...
                "design_target": (np.squeeze([np.repeat(y_bar.item(), n_runs*n_seqs) for y_bar in y_bars])).flatten(),
            ...
            with open(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_Ledidi_design_pos-{midpoint}_flanks-{fl}_{run_hash}.tsv"), 'w', newline='') as f:
            ...
            # Save X_optim to npz
            np.savez_compressed(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_Ledidi_X_optim_pos-{midpoint}_flanks-{fl}_{run_hash}.npz"), X_optim=X_optim.numpy(force=True))
