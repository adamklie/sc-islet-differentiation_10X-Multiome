    ...
    for midpoint in seqlet_span_midpoints:
        for fl in flank_choices:
            ...
            for y_bar in tqdm(y_bars, desc="Running ISM for each decile"):
                print(f"Running ISM for decile target: {y_bar.item()} in positions ({edit_start}-{edit_end}")
                X_hat_ = []
            ...
            for decile in range(X_hat.shape[0]):
                for run in range(X_hat.shape[1]):
                    for t, seq in enumerate(range(X_hat.shape[2])):
                        identifiers.append(f"ISL1_{chrom_}:{start_}-{end_}_ISM_run-{run+1}_target-{round(y_bars[decile].item(), 2)}_pos-{midpoint}_flanks-{fl}_{t+1}")
            ...
            for j, y_bar in enumerate(y_bars):
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_ISM_design_target_{round(y_bar.item(), 2)}_{run_hash}.pdf"), dpi=300)
                plt.show()
            ...
            np.savez_compressed(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_ISM_X_ref_attr_pos-{midpoint}_flanks-{fl}_{run_hash}.npz"), X_ref_attr=X_ref_attr_.numpy(force=True))
            np.savez_compressed(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_ISM_X_optim_attr_pos-{midpoint}_flanks-{fl}_{run_hash}.npz"), X_optim_attr=X_optim_attr_.numpy(force=True))
            ...
            seqlets[["id", "start", "end", "attribution", "p-value", "annotation"]].sort_values(["id", "start"]).to_csv(
                os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_ISM_seqlets_pos-{midpoint}_flanks-{fl}_{run_hash}.tsv"), 
                sep="\t", 
                index=False
            )
            ...
            delta_seqlets_all.sort_values(["id", "start"]).to_csv(
                os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_ISM_delta_seqlets_pos-{midpoint}_flanks-{fl}_{run_hash}.tsv"), 
                sep="\t", 
                index=False
            )
            ...
            # Save edit_df
            edit_df[["id", "chrom", "pos", "ref", "alt", "variant_id", "relative_pos"]].to_csv(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_ISM_edits_pos-{midpoint}_flanks-{fl}_{run_hash}.tsv"), sep="\t", index=False)
            ...
            with open(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_ISM_design_{run_hash}.tsv"), 'w', newline='') as f:
            ...
            # Save X_optim to npz
            np.savez_compressed(os.path.join(path_out, dir_name, subdir_name, f"ISL1_{chrom_}:{start_}-{end_}_ISM_X_optim_pos-{midpoint}_flanks-{fl}_{run_hash}.npz"), X_optim=X_optim.numpy(force=True))
