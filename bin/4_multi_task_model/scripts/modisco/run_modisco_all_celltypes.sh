#!/bin/bash
# Run MoDISco on peak regression contributions (one task per cell type).
# Expects per-CT NPZ outputs from compute_contributions_specific_v2.py:
#   {ct}_contrib.npz  — (N, 4, L) hypothetical attributions (tutorial "hypothetical")
#   {ct}_oh.npz       — (N, 4, L) one-hot sequences
# Computes projected = contrib * oh on the fly.
#
# USAGE:
#   sbatch --job-name=modisco_celltypes \
...
CONTRIBS=${BASE}/crested/contributions_specific_v2
OUT=${BASE}/crested/modisco/${CT}
...
# Step 2: modisco motifs
echo "Step 2: modisco motifs ${CT}"
$MODISCO_BIN motifs -i ${OUT}/${CT}_contribs.h5 -n 50000 -o ${OUT}/${CT}.modisco.h5 -l 2 -w 400 -v
