#!/bin/bash
#####
# Per-cell-type contribution scoring on finetuned_v2 (peak regression) — v3 run.
# Difference vs v2: --top_k_regions 2000 (tutorial fidelity) and writes to
# results/4_multi_task_model/crested/contributions_specific_v3/.
# Reuses the existing predictions+combined-layer AnnData built by predict_and_save_adata.sh
# (AnnData itself does not depend on top_k).
#
...
#     --array=1-22%4 --job-name=pr_contrib_spec_v3 \
...
mkdir -p ${BASE}/crested/contributions_specific_v3
...
python ${SCRIPT_DIR}/compute_contributions_specific_v2.py \
    ...
    --output_dir ${BASE}/crested/contributions_specific_v3 \
    --cell_types ${celltype} \
    --top_k_regions 2000 \
