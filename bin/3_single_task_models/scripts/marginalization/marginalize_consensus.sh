#!/bin/bash
#####
# Marginalize the `consensus` MEME (76 cluster consensus motifs) across all 22
# per-cell-type ChromBPNet models. Explicit python (conda activate fails in batch).
# USAGE:
# sbatch \
# --job-name=marg_consensus \
# --account carter-gpu \
...
# -t 14-00:00:00 \
# --array=1-22%10 \
# --output /carter/users/aklie/projects/islet_organoid_differentiation/sc-islet-differentiation_10X-Multiome/bin/slurm_logs/3_single_task_models/marginalization/consensus/%x.%A.%a.out \
# marginalize_consensus.sh
#####
set -e
...
MOTIF=$MROOT/motifs/gimme_cluster/marginalization/consensus/query.meme
...
outdir=$MROOT/$ct/fold_0/chrombpnet/0.5/marginalization/consensus
echo -e "CT $ct\nmodel $model\npeak $peak\nout $outdir\n"; mkdir -p $outdir
